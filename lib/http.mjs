...
export const basliktanOku = (req, ad) => {
  try { return decodeURIComponent(String(req.headers[ad] || '')); } catch { return ''; }
};

/**
 * Yüklenmiş dosyayı İNDİRME olarak gönderir: her zaman ek, sandbox başlığıyla
 * ve önbelleksiz. Dosyalar arasında SVG ve HTML'e benzeyen belgeler de
 * olabilir; aynı kökende tarayıcıda açılmamalı (klasörlerdeki karar).
 */
export function ekGonder(req, res, { ad, tur, veri }) {
  const govde = Buffer.from(veri);
  const ascii = ad.replace(/[^\x20-\x7e]/g, '_').replace(/["\\]/g, '_');
  res.writeHead(200, {
    'Content-Type': tur, 'Content-Length': govde.length, 'Cache-Control': 'private, no-store',
