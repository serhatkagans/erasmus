import { parcaKlasoru } from './klasor.mjs';
...
export async function dosyaOzetleri(db) {
  const olaylar = await db.all('SELECT id,slug,tur FROM events');
  const dosyalar = await db.all('SELECT DISTINCT event_id, tur FROM event_files');
  /* Bağ kurulmadan önce toplantı klasörüne (05.x) yüklenmiş belgeler de
     parçayı tamamlar: yüklenmiş bir tutanak "eksik" görünmemeli. */
  const eskiKlasor = new Set((await db.all('SELECT DISTINCT klasor FROM klasor_dosya')).map(r => r.klasor));
  ...
  for (const e of olaylar) {
    ...
    const tamamMi = p => (p === 'foto' ? fotolar.has(e.id) : p === 'anket' ? anketler.has(e.id) : var_.has(`${e.id}:${p}`))
      || eskiKlasor.has(parcaKlasoru(e.slug, p));
