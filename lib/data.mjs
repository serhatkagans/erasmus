...
/**
 * Görevin durumu ile ilerlemesini tutarlı tutar: `[durum, ilerleme]`.
 *
 ...
 * Uçlar birbirine bağlıdır: tamamlanan görev %100, bekleyen ve iptal
 * edilen görev %0; aradaki değerler "devam ediyor"dur.
 *
 * DEĞİŞTİRİLEN ALAN KAZANIR (`onceki` verilince). Eskiden kural hep aynı
 * yöndeydi: %100'deki bir görevi "devam ediyor"a çekmek istek %100'ü de
 * taşıdığı için yeniden "tamamlandı"ya dönüyordu, kaydırıcıyı %100'den
 * aşağı çekmek de "tamamlandı" durumu yüzünden %100'e geri sıçrıyordu —
 * tamamlanan görev bir daha açılamıyordu (24 Eylül 2026 kullanıcı bildirimi).
 */
export function gorevDurumu(durumDegeri, ilerlemeDegeri, onceki = null) {
  let durum = requireOneOf(durumDegeri, taskStatuses, 'Durum');
  let ilerleme = requireProgress(ilerlemeDegeri ?? 0);
  const durumDegisti = !!onceki && durum !== onceki.durum;
  const ilerlemeDegisti = !!onceki && ilerleme !== Number(onceki.ilerleme ?? 0);

  if (ilerlemeDegisti && !durumDegisti) {
    /* Kaydırıcı oynatıldı: durum ilerlemeden çıkar. */
    if (ilerleme === 100) durum = 'tamamlandi';
    else if (durum === 'tamamlandi' || durum === 'iptal' || durum === 'bekliyor') durum = ilerleme > 0 ? 'devam' : 'bekliyor';
    return [durum, ilerleme];
  }
  if (durumDegisti && !ilerlemeDegisti) {
    /* Durum seçildi: ilerleme duruma uyar. Yeniden açılan görev tamamlanmış
       sayılmasın diye bir adım geri çekilir. */
    if (durum === 'tamamlandi') ilerleme = 100;
    else if (durum === 'bekliyor' || durum === 'iptal') ilerleme = 0;
    else if (ilerleme === 100) ilerleme = 100 - PROGRESS_STEP;
    return [durum, ilerleme];
  }
  /* Yeni görev ya da ikisi birden değişti (düzenleme formu): uçlar bağlanır. */
  if (durum === 'tamamlandi') ilerleme = 100;
