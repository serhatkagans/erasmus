...
/* --- Faaliyet dosyasına bağlı klasörler -------------------------------------
   Toplantı klasörlerinin (05.x) iç klasörleri faaliyet dosyasının
   parçalarıyla aynı belgeleri tutar. Eskiden iki ayrı depoydu: klasöre
   konan tutanak panoda "eksik" görünüyordu ve aynı belge iki yere
   yükleniyordu. Artık bu iç klasörlerin içeriği FAALİYET DOSYASIDIR
   (lib/faaliyet.mjs): klasöre yükleme de faaliyete yazar. "Sunum"un
   faaliyet dosyasında karşılığı yok, sıradan klasör olarak kalır.

   Bağ yalnızca faaliyetin türü o parçayı istiyorsa kurulur: sanal
   toplantıda bilgi paketi ve fotoğraf beklenmez, o iç klasörler sıradan
   klasördür. Tür veritabanındadır; bu yüzden burada yalnızca ADAY bağ
   verilir, kesin karar route'ta türle birlikte verilir. */
export const IC_PARCA = { ajanda: 'gundem', infopack: 'infopack', katilim: 'yoklama', tutanak: 'tutanak', foto: 'foto', anket: 'anket' };

/** {etkinlik: slug, parca} ya da null. */
export function bagliParca(klasorId) {
  const k = DIZIN.get(klasorId);
  if (!k || k.etiket !== 'ic' || !IC_PARCA[k.ic]) return null;
  const ust = DIZIN.get(k.ust);
  return ust?.etkinlik ? { etkinlik: ust.etkinlik, parca: IC_PARCA[k.ic] } : null;
}

/** Bir faaliyetin bir parçasının bağlı olduğu klasör (varsa). */
export function parcaKlasoru(slug, parca) {
  return KLASORLER.find(k => { const b = bagliParca(k.id); return b && b.etkinlik === slug && b.parca === parca; })?.id || null;
}
